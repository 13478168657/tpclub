...
$.fn.UploadImg = function(o){
    this.change(function(){
        ...
        if($(this).hasClass('cover')){
            var className = 'cover_img';
        }else{
            var className = 'explain_img';
        }
        ...
        if(file.size && file.size > o.mixsize){
            ...
            // o.before(blob);
            _compress(blob,file,className,this);
            this.value='';
        }
    });


    function _compress(blob,file,className,obj){
        var img = new Image();
        img.src = blob;
        ...
        img.onload = function(){
            ...
            o.before(base64,className);
             _ajaximg(base64,file.type,className, obj);
        };
    }

    function _ajaximg(base64,type,className,obj){
        var token = $("input[name='_token']").val();
        $.post(o.url,{file:base64,type:type,_token:token},function(res){
            if(res.code == 0){
                o.success(res.url,className,obj);
            }else{
                alert(res.message);
            }
        });
