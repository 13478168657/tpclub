...
<div class="text_center fz">
    <div class="bgban1">
        ...
        @else
            ...
            @else
                <h3 class="bold f38 pb30">还差{{$courseClassGroup->team_people - count($classJoins)}}人，赶快邀请好友完成拼团</h3>
    ...
    <div class="bgban2">
        <ul class="clearfix">
            ...
            @endforeach
            @if($k == 0)
                @if($courseClassGroup->team_people - ($k+1) == 1)
                    <li>
                        <div class="pImg">
    ...
    <div class="nowPin">
        ...
        <div class="mt70 text_left txt">
            <h3 class="bold f30 pb20 mb30">{{$courseClassGroup->title}}（第{{$orderCourse->stage}}期）</h3>
            <?php
                $teamDesc = explode("\n",$courseClassGroup->team_course_desc);
            ?>
    ...
    <div class="relative">
        @if($courseClassGroup->team_people - count($classJoins) == 0)
            <a href="/train/success" class="pPosBtn f36">报名成功 完善报名信息</a>
        @else
            @if(strtotime($orderCourse->uneffect_time)< time())
...
<script type="text/javascript">
    ...
    wx.ready(function () {   //需在用户可能点击分享按钮前就先调用
        wx.onMenuShareAppMessage({
            ...
            desc: desc, // 分享描述
            link: "http://m.saipubbs.com/train/join/{{$fission_id}}s{{$courseClassGroup->id}}.html", // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
