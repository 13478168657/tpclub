...
<head>
    ...
    {{--本页面css--}}
    <link rel="stylesheet" href="/css/zt/zt_bigclass.css?t=1.01">
        ...
        <div class="content">
            ...
            <div class="ptb20">
                <div class="zaoniao_btn mt60 plr25 clearfix text_center relative">
                    {{--<a href="javascript:void (0)" data-stage="1" onclick="zao_show1(this);" class="button border-radius50 fz f24 bga">10月9号开班</a >--}}
                    <a href="javascript:void (0)" data-stage="1" data-price="4800" onclick="zao_show2(this);" class="button border-radius50 fz f24 bga">11月15日北京校区</a >
                    <a href="javascript:void (0)" data-stage="2" data-price="5600" onclick="zao_show3(this);" class="button border-radius50 fz f24 ">12月13日北京校区</a >
                </div>
            </div>
            <!--按钮切换 end-->
        ...
        <div class="foot plr20 bgcolor_fff">
            <div class="foot_u">
                <ul class="clearfix">
                    ...
                    <li class="leftW clearfix plr20" id="zao2">
                        <div class="fl nth1">
                            <span class="block fz f22 bold">11月15日开班</span>
                            <span class="block fz f22 bold">还剩<span class="color_be3f00">{{$stageNumArr[1]}}</span>个名额</span>
                        </div>
                        @if($add_info)
                            <div class="fr nth2" onclick="javascript:window.location='/activeCourse/addUserInfo/{{$data->id}}.html?fid={{$fission_id}}'">
                        ...
                        @else
                            <div class="fr nth2" onclick="javascript:window.location='/activeCourse/addUserInfo/{{$data->id}}.html?fid={{$fission_id}}'">
                                <img src="/images/wanshan.png" alt="">
                                <span class="fz f22 block text_center">请完善</span>
                            </div>
                        @endif
                    </li>
                    <li class="leftW clearfix plr20 none" id="zao3">
                        <div class="fl nth1">
                            <span class="block fz f22 bold">12月13日开班</span>
                            <span class="block fz f22 bold">还剩<span class="color_be3f00">{{$stageNumArr[2]}}</span>个名额</span>
                        </div>
                        @if($add_info)
                            <div class="fr nth2" onclick="javascript:window.location='/activeCourse/addUserInfo/{{$data->id}}.html?fid={{$fission_id}}'">
                                <img src="/images/wanshan.png" alt="">
                    ...
                    <li class="rightW bgcolor_orange border-radius-img plr20">
                        <div class="clearfix text_center">
                            @if($mobile==0)
                                <button class="fz color_5c2a0c open-popup" onclick="userlogin();">
                                    <span class="f43 mr20 color_be3f00"><span class="changePrice">{{$price}}</span><b class="f19">元</b></span>
                                    <span class="f30 f30">马上报名</span>
                                </button>
                            @else
                                @if($is_buy)
                                    <button class="fz color_5c2a0c open-popup">
                                        <span class="f30 f30">已报名</span>
                                        @else
                                            @if($add_info)
                                                <button class="fz color_5c2a0c open-popup" data-target="#half">
                                                    <span class="f43 mr20 color_be3f00"><span class="changePrice">{{$price}}</span><b class="f19">元</b></span>
                                                    <span class="f30 f30">马上报名</span>
                                                    @else
                                                        <button class="fz color_5c2a0c open-popup">
                                                            <span class="f43 mr20 color_be3f00"><span class="changePrice">{{$price}}</span><b class="f19">元</b></span>
                                                            <span class="f30 f30" onclick="javascript:window.location='/activeCourse/addUserInfo/{{$data->id}}.html?fid={{$fission_id}}'">马上报名</span>
                                                            @endif
                                                            @endif
                                                        </button>
                                @endif
            ...
            <div class="modal-content bgc_white">
                <div class="mor_list fz color_333 plr30 ptb30">
                    <div class="weui-cell  mt0 padding0">
                        ...
                        <div class="weui-cell__ft price color_333 bold f28 bold bugPrice">{{$price}}元</div>
                    </div>
...
<script>
    ...
    function jsApiCall()
    {
        ...
        var data = {class_id:c_c_id,fission_id:fission_id,stage:stage,_token:_token};
        $.ajax({
        ...
        $('.payBtn').click(function (){
            ...
            if(payfrom=='BANLANCE'){
                ...
                if(is_weixin==1){
                    ...
                    $.ajax({
                        type:"POST",
                        url:"/activeCourse/payh",
                        data:{course_class_id:c_c_id,fission_id:fission_id,stage:stage,_token:token},
                        dataType:"json",
                        success:function(result){
                            if(result.code==1){
