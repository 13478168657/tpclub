...
<script type="text/javascript">
    ...
    <?php
        $content = explode(PHP_EOL,$course->seo_description);
        $art = '';
    ...
    wx.ready(function () {   //需在用户可能点击分享按钮前就先调用
        wx.onMenuShareAppMessage({
            title: '{{$course->title}}', // 分享标题2018年终锦鲤免费预定注册私教
            desc: '{{$art}}', // 分享描述
            link: "http://m.saipubbs.com/course/detail/{{$course->id}}.html?fission_id={{$user_id}}", // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
            imgUrl:  "{{env('IMG_URL')}}{{$course->cover_url}}", // 分享图标
        }, function(res) {
        //这里是回调函数
        });
    });
