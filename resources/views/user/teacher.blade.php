...
<div class="bgc_white jianjieCon">
	<p class="">{{$user->introduction ? $user->introduction : '暂无简介'}}</p>
	<!-- 没有简介就隐藏按钮 -->
			...
			<div class="list plr30 bgcolor_fff" id="list">
				<ul>
					@foreach($list as $course)
						<li class="ptb30">
							<dl class="clearfix">
								<dt class="border-radius-img"><img src="{{env('IMG_URL')}}{{$course->cover_url}}" alt="" /><span class="bgcolor_orange text_center fz color_333">{{$course->cytitle}}</span></dt>
								<dd>
									<h3 class="lt f30">{{$course->title}}</h3>
...
<script>
    ...
    $(document).ready(function() {
		...
		$('#add_more').click(function(){
			var data = {type:1,page:1,'userId':user_id};
			$.ajax({
