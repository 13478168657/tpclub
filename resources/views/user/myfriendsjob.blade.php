		...
		@foreach($list as $a)
				<div class="weui-cells nobefore  mt0 myfriend-list">
					<a class="weui-cell weui-cell_access ptb44 nobefore noafter" href="javascript:;">
						...
						<p class="f32 bold color_333 pl20">{{$a->name ? subtext($a->name,6) : "信息未完善"}}</p>
					</div>
					<div class="weui-cell__ft f24 color_333 noafter no-padding">
						<p class="fz bold">{{$a->mobile?$a->mobile:'--'}}</p>
						<span class="color_gray9b f20">{{$a->created_at}}</span>
						<p class="fz bold">邀请人:{{$a->fisson_name ? $a->fisson_name : '暂无昵称'}}</p>
					</div>
					</a>
				</div>
		@endforeach
                ...
                success:function(result){
                    ...
                    if(result.code==1){
                        for (var item in result.list) {
                            ...
                            if(result.list[item].mobile){
                            	var mobile = result.list[item].mobile;
                            }else{
                            	var mobile = "--";
                            }
                            if(result.list[item].fisson_name){
                            	var fisson_name = result.list[item].fisson_name;
                            }else{
                            	var fisson_name = "--";
                            }
