...
   
    ...
    <div class="distance plr30">
            ...
            <ul class="clearfix dis_head1 mt30 pt30">
                ...
                @if(isset($sponsor->uid))
                    ...
                    ?>
                     ...
                     @endforeach
                     @for ($i = 0; $i < 3-$num; $i++)
                            <li class="fl" @if($i == 0) id = "img_replace" @endif><img src="{{$moren}}" alt=""></li>
                     @endfor
                @else
        ...
        <p class="f26 sy_r text_center mt40 dist_txt1 mb100"><i></i>还差<b class="numshare">{{3-count($data)}}</b>位好友即可免费领取<i></i></p>
    </div>
    <!-- 距离活动结束还有 end -->
...
<script>
    ...
    $("#friend_zhuli").click(function(){
        ...
        if(status == 0){
            ...
            var data = {userid:userid,friend:friend,aid:aid,cid:cid,_token:token};
            console.log(data);
            $.ajax({
                url:'/newyear/zhuli',
                type:'POST',
                ...
                success:function(data){
                    if(data.code == 1){
                        ...
                        @endif
                        $("#img_replace img").attr("src",avatar);
                        $("#friend_zhuli").attr("data-attr",1);
                        var newnum = $(".numshare").text();
                        $(".newnum").text(newnum-1);
                    }else if(data.code == 3){
