   ...
   <!-- 福利课程：4节课 马上从健身菜鸟变为为健身达人  end -->
    ...
    <div class="mt30 mlr30 list bg_e16354 border-radius-img">
        <div class="bd">
            <ul class="f24 sy_r">
                <li class="clearfix">
                    <p class="fl text-overflow">ID 李雅婷最美 ... </p>
                ...
                <li class="clearfix">
                    <p class="fl text-overflow">ID 成长的橙子 ... </p>
                    <span class="fr text_right">领取该福利</span>
                </li>
                <li class="clearfix">
                ...
                <div class="dis_head relative">
                    ...
                    @if(isset($sponsor->uid))
                        ...
                        @for ($i = 0; $i < $anum-$num; $i++)
                            <img @if($i == 0) id = "img_replace" @endif src="/images/new_year/head_img2.png" alt="" class="head_img{{$anum-$i+1}}">
                        @endfor
                    @else
 ...
 @elseif($aid == 4||$aid == 5)
    ...
    <div class="distance plr30">
        ...
        <div class="clearfix dis_con mt40 distance_hb mb26">
            @if(isset($sponsor->uid))
                ...
                $num = count($data);
                ?>
                <p class="fl f28 sy_b color_7a4300"><img src="{{$name->avatar?$name->avatar:'/images/new_year/head_img1.png'}}" alt="">{{$name->nickname?$name->nickname:'小伙伴们..'}}</p>
            @else
                <p class="fl f32 sy_b color_7a4300">小伙伴们..</p>
            @endif
            ...
            <div class="bd">
                <ul class="">
                    ...
                    @if(isset($sponsor->uid))
                        ...
                        $num = count($data);
                        ?>
                        <li class="clearfix distance_hb distance_hb1 mb26 friend_append">
                            <p class="fl f28 sy_b  text-overflow color_7a4300"><img src="{{$avatar?$avatar->avatar:$moren}}" alt="">{{$avatar->nickname?$avatar->nickname:'小伙伴们..'}}</p>
                        ...
                        @if($num > 0)
                            @foreach($data as $k=>$v)
                                ...
                                <li class="clearfix distance_hb distance_hb1 mb26">
                                    <p class="fl  text-overflow f28 sy_b color_7a4300"><img src="{{$avatar?$avatar->avatar:$moren}}" alt="">{{$avatar?$avatar->nickname:'小伙伴们..'}}</p>
                                    <span class="fr text_right text-overflow f26 sy_r">已经助力…</span>
                                </li>
                            @endforeach
                        @endif
    ...
    <div class="activity mlr30">
        ...
        <div class="bg_c93126 border-radius-img act_txt color_fff mt30">
            <p class="f28 sy_r mb70">
                01、如何参与活动：<br>点击页面底部【邀请好友助力】分享到朋友圈或者微信好友寻求{{$anum}}位好友助力。
            </p>
            <p class="f28 sy_r mb70">
    ...
    <div class="activity mlr30">
        ...
        <div class="bg_c93126 border-radius-img act_txt color_fff mt30">
            ...
            <p class="f28 sy_r mb70">
                02、如何领取福利：<br>成功邀请25名好友后，在本页面点击【领取福利】，复制卡号和卡密，用电脑登陆京东商城，点击首页导航栏【我的京东】-左侧【礼品卡】，输入卡密即可绑定；本卡仅能在京东商城使用哦~
            </p>
            <p class="f28 sy_r mb70">
                03、如何查询好友助力：<br>在本页面点击【领取福利】查询好友助力情况，或到公众号【赛普健身社区】-【免费福利】-【领取奖励】-进入活动页面查询助力情况或领取福利。
            </p>
...
<br><br><br>
    ...
    <div class="fixed_wrap">
        <ul class="clearfix sy_m f28 text_center">
            @if($userid == 0)
                <li class="color_333" onclick="userlogin()"><a href="javascript:;">马上助力</a></li>
                <li class="color_333" onclick="userlogin()"><a href="javascript:;">我也要领取</a></li>
            @else
                    ...
                    @if($sponsor_count <= $maxnum)
                        ...
                        @else
                            @if($num < $anum)
                                <li class="color_333"><a href="/newyear/sharecard/{{$aid}}?userid={{$userid}}">邀请好友助力</a></li>
                        ...
                        @endif
                    @else
                        <li class="color_333" onclick="layer.msg('来晚啦 礼品已被领光!');"><a href="javascript:;">邀请好友助力</a></li>
                        <li class="color_333" onclick="layer.msg('来晚啦 礼品已被领光!');"><a href="javascript:;">领取福利</a></li>
                    @endif
                    @else
                    @if($sponsor_count < $maxnum)
                        @if($mobile == 0)
                            <li class="color_333" onclick="userlogin()"><a>马上助力</a></li>
                            <li class="color_333"><a href="/newyear/index.html">我也要领取</a></li>
                        @else
                            <li class="color_333" id = "friend_zhuli" data-attr = "0"><a>马上助力</a></li>
                            <li class="color_333"><a href="/newyear/index.html">我也要领取</a></li>
                        @endif

                    @else
                        <li class="color_333" onclick="layer.msg('来晚啦 礼品已被领光!');"><a href="javascript:;">马上助力</a></li>
                        <li class="color_333" onclick="layer.msg('来晚啦 礼品已被领光!');"><a href="javascript:;">我也要领取</a></li>
                    @endif
                    @endif
...
<script>
    ...
    $("#friend_zhuli").click(function(){
        var status = $(this).attr("data-attr");

        if(status == 0){

            var token = '{{csrf_token()}}';
            var data = {userid:userid,friend:friend,aid:aid,_token:token};
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.code == 1){
                        ...
                        @if($userid !== 0)
                            <?php
                                $avatar = DB::table("users")->where("id",$userid)->select("avatar","nickname")->first();
                            ?>
                            @if($aid == 2||$aid == 3)
                            var avatar = "{{$avatar?$avatar->avatar:'/images/new_year/head_img2.png'}}";
                            $("#img_replace").attr("src",avatar);
                            @else
                                var str = '<li class="clearfix distance_hb distance_hb1 mb26"><p class="fl text-overflow f28 sy_b color_7a4300"><img src="{{$avatar?$avatar->avatar:$moren}}" alt="">{{$avatar->nickname?$avatar->nickname:"小伙伴们.."}}</p> <span class="fr text_right text-overflow f26 sy_r">已经助力…</span> </li>';
                                $(".friend_append").after(str);
                            @endif
                        @endif
                        $("#friend_zhuli").attr("data-attr",1);
                    }else if(data.code == 3){
                        layer.msg("你只能帮别人助力一次哦，快来自己领取课程吧！");
                    }else if(data.code == 4){
                        layer.msg("不能给自己助力哦，快去邀请好友吧！");
                    }else if(data.code == 5){
                        layer.msg("抱歉新用户才可助力 点右侧【领取福利】参与");
                    }else{
                        layer.msg("您已经助力过了哦");
                    }
                }
